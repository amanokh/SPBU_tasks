...
int main() {
    ...
    for (int step = 0; step < min(m, n); ++step) {
        if (step % 2 == 0) {
            ...
            for (int j=0; j < m - step - 1; ++j) b[first + j + 1][n - step / 2] = b[first + j][n - step / 2] + 1; //по столбцу вниз
        } else {
            int iStart = m - step / 2;
            int jStart = n - step / 2 - 1;
            for (int i=0; i < n - step; ++i) b[iStart][jStart - i] = b[iStart][jStart - i + 1] + 1; //по строчке влево
            for (int j=0; j < m - step - 1; ++j) b[iStart - j - 1][step / 2 + 1] = b[iStart - j][step / 2 + 1] + 1; //по столбцу вверх
        }
    }
